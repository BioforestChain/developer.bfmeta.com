...
本章将介绍在使用生物链林数据中心版BCF.exe时可能遇到的常见问题的解决方案。若您遇到了本章中未列出的问题时，请与生物链林BCF团队(邮箱地址：service@bfmeta.org)联系以获得更多的支持。
...
1. 运行后修改配置文件实现自动投票功能
    ...
    可根据\<[设置节点配置信息](/zh/API参考/PC全节点/接口列表/1-4.md#15.设置节点配置信息)\>的自动投票字段在节点运行过程中，动态调整配置信息。
    如下图所示，可开启自动投票。
...
1. 使用\<[获取节点状态接口](/zh/API参考/PC全节点/接口列表/1-2.md#7.获取节点状态)\>，查看节点的状态。

2. 使用\<[获取本地节点当前最新区块](/zh/API参考/PC全节点/接口列表/1-2.md#2.获取本地节点当前最新区块)\>，观察本节点是否能与其他节点保持一致的高度，若可以则节点运行正常。
